...
//
// Description: This file implements a derived class of ClientSecurityContext which is 
//              a SSPI wrapped class.
//
//
//------------------------------------------------------------------------------
    ...
    public class KerberosClientSecurityContext : ClientSecurityContext, IDisposable
    {
        ...
        public override void Initialize(byte[] inToken)
        {
            if (inToken == null || inToken.Length == 0)
            {
                ...
                _SeqOfPA_DATA paData = client.ConstructPaData(timestamp, pacRequest);
                    ...
                    == ClientSecurityContextAttribute.DceStyle)
                {
                    ...
                    subkey = new EncryptionKey((int)EncryptionType.RC4_HMAC,
                        KileUtility.GenerateRandomBytes(ConstValue.RC4_KEY_LENGTH));
                }
                else
                {
                    asRequest = client.CreateAsRequest(sname, flags, paData, EncryptionType.RC4_HMAC);
                ...
                KileAsResponse asResponse = (KileAsResponse)client.ExpectPdu(ConstValue.TIMEOUT_DEFAULT);
                ...
                KileTgsRequest tgsRequest = client.CreateTgsRequest(sname,
                                                              flags,
                                                              new UInt32((uint)Math.Abs((int)DateTime.Now.Ticks)),
                                                              null,
                                                              ChecksumType.hmac_md5_string,
                                                              null,
                                                              null);
                client.SendPdu(tgsRequest);
                KileTgsResponse tgsResponse = (KileTgsResponse)client.ExpectPdu(ConstValue.TIMEOUT_DEFAULT);
