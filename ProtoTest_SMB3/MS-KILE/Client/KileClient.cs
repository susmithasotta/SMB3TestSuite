...
//
// Description: This file implements KILE client, which receives server  
//              PDUs and sends client PDUs.
//
//------------------------------------------------------------------------------
...
namespace Microsoft.Protocols.TestTools.StackSdk.Security.Kile
{
    ...
    public class KileClient : KileRole
    {
        ...
        /// <param name="encryptionTypes">The encryption types client supported. This argument cannot be null.
        /// If this argument is null, NullReferenceException will be thrown.</param>
        /// <returns>The created AS request.</returns>
        public KileAsRequest CreateAsRequest(string sname,
                                         KRBFlags kdcOptions,
                                         _SeqOfPA_DATA paData,
                                         params EncryptionType[] encryptionTypes)
        {
            ...
            request.Request.req_body.nonce = new UInt32((uint)Math.Abs((int)DateTime.Now.Ticks));
            ...
            request.Request.req_body.addresses =
                new HostAddresses(new HostAddress[1] { new HostAddress((int)AddressType.NetBios,
                    Encoding.ASCII.GetBytes(Dns.GetHostName())) });

            if (userName != null)
            {
                request.Request.req_body.cname =
                    new PrincipalName((int)PrincipalType.NT_PRINCIPAL, KileUtility.String2SeqKerbString(userName));
            }
            ...
            if (sname != null)
            {
                request.Request.req_body.sname =
                    new PrincipalName((int)PrincipalType.NT_SRV_INST, KileUtility.String2SeqKerbString(sname, domain));
            }

            if (encryptionTypes != null)
            {
                Int32[] etypes = new Int32[encryptionTypes.Length];
                for (int i = 0; i < encryptionTypes.Length; i++)
                {
                    etypes[i] = new Int32((int)encryptionTypes[i]);
                }

                request.Request.req_body.etype = new _SeqOfInt32(etypes);
            }
        ...
        /// <param name="aditionalTicket">An aditional ticket of request body. This field is optional.
        /// This argument can be null. If this argument is null, no aditional ticket will be sent.</param>
                                           ...
                                           AuthorizationData authorizationData)
        {
            ...
            request.Request.req_body.realm = context.UserRealm;

            if (additionalTicket != null)
            {
                request.Request.req_body.additional_tickets = new _SeqOfTicket(new Ticket[] { additionalTicket });
            }

            if (sname != null)
            {
                request.Request.req_body.sname =
                    new PrincipalName((int)PrincipalType.NT_SRV_INST,
            ...
            {
                Asn1BerEncodeBuffer asnBuffer = new Asn1BerEncodeBuffer();
                authorizationData.Encode(asnBuffer, true);

                request.Request.req_body.enc_authorization_data = new EncryptedData();
                request.Request.req_body.enc_authorization_data.etype = new Int32(0);
                byte[] encAsnEncoded = asnBuffer.MsgCopy;
                if (context.TgsSessionKey != null && context.TgsSessionKey.keytype != null
                    && context.TgsSessionKey.keyvalue != null && context.TgsSessionKey.keyvalue.mValue != null)
                {
                    encAsnEncoded = KileUtility.Encrypt((EncryptionType)context.TgsSessionKey.keytype.mValue,
                                                        context.TgsSessionKey.keyvalue.mValue,
                                                        asnBuffer.MsgCopy,
                                                        (int)KeyUsageNumber.TGS_REQ_KDC_REQ_BODY_AuthorizationData);
                    request.Request.req_body.enc_authorization_data.etype =
                        new Int32(context.TgsSessionKey.keytype.mValue);
                }
            ...
            Asn1BerEncodeBuffer bodyBuffer = new Asn1BerEncodeBuffer();
            request.Request.req_body.Encode(bodyBuffer);
            PA_DATA tgsPaData = ConstructTgsPaData(checksumType, bodyBuffer.MsgCopy);

            request.Request.padata = new _SeqOfPA_DATA();
                                         ...
                                         AuthorizationData authorizationData)
        {
            ...
            request.Authenticator = CreateAuthenticator(checksumType,
                                                        seqNumber,
                                                        flag,
                                                        subkey,
                                                        authorizationData,
        ...
        public KrbCred CreateKrbCredRequest()
        {
            ...
            cred.KerberosCred.tickets = new _SeqOfTicket(ticket);
        ...
        public _SeqOfPA_DATA ConstructPaData(params PaData[] paData)
        {
        ...
        public EncryptionKey GenerateKey(EncryptionKey baseKey)
        {
            if (baseKey == null || baseKey.keytype == null
                || baseKey.keyvalue == null || baseKey.keyvalue.mValue == null)
            {
                return null;
            }

            byte[] keyBuffer = KileUtility.GenerateRandomBytes((uint)baseKey.keyvalue.Length);
            EncryptionKey newKey = new EncryptionKey(baseKey.keytype.mValue, keyBuffer);
            return newKey;
        }
        ...
        private Authenticator CreateAuthenticator(ChecksumType checksumType,
                                                  int seqNumber,
                                                  ChecksumFlags flag,
                                                  EncryptionKey subkey,
                                                  AuthorizationData authorizationData,
                                                  ...
                                                  byte[] checksumBody)
        {
            ...
            plaintextAuthenticator.seq_number = new UInt32(seqNumber);
            ...
            if (checksumType == ChecksumType.ap_authenticator_8003)
            {
                ...
                // in AP request
                plaintextAuthenticator.cksum = new Checksum((int)checksumType, checkData);
            }
            else
            {
                // in TGS PA data
        ...
        private PA_DATA ConstructTgsPaData(ChecksumType checksumType, byte[] checksumBody)
        {
            ...
            Authenticator authenticator = CreateAuthenticator(checksumType,
                                                              0,
                                                              0,
                                                              null,
                                                              authData,
            ...
            byte[] encAsnEncodedAuth =
                KileUtility.Encrypt((EncryptionType)context.TgsSessionKey.keytype.mValue,
                                    context.TgsSessionKey.keyvalue.mValue,
            ...
            request.authenticator.etype = new Int32(context.TgsSessionKey.keytype.mValue);
            ...
            request.Encode(apBerBuffer, true);

            return new PA_DATA((int)PaDataType.PA_TGS_REQ, apBerBuffer.MsgCopy);
        }
        ...
        private EncKrbCredPart ConstrutCredEncryptedData(EncryptionKey key)
        {
            ...
            encKrbCred.nonce = new UInt32((uint)Math.Abs((int)DateTime.Now.Ticks));
            encKrbCred.timestamp = new KerberosTime(KileUtility.GetCurrentUTCTime());
            encKrbCred.usec = new Microseconds(0);
            encKrbCred.s_address = new HostAddress(new Int32((int)AddressType.NetBios),
            ...
            encKrbCred.ticket_info = new _SeqOfKrbCredInfo(krbCredInfo);
        ...
        private EncKrbPrivPart ConstructEncKrbPrivPart(KRB_PRIV_REQUEST krbPrivRequest, byte[] userData)
        {
            ...
            encKrbPriv.s_address = new HostAddress(new Int32((int)AddressType.NetBios),
                                                   new Asn1OctetString(Dns.GetHostName()));
            ...
            {
                encKrbPriv.seq_number = new UInt32((long)context.CurrentLocalSequenceNumber);
