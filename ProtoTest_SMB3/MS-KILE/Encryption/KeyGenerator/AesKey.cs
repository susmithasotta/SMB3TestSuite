...
// All rights reserved.
//
// Module Name: AES Key Class
//
// Abstract:
// Provide methods for AES Key Generation in MS-KILE
//
// Reference:
...
namespace Microsoft.Protocols.TestTools.StackSdk.Security.Kile
{
    ...
    internal static class AesKey
    {
        ...
        private static byte[] GetWellKnownConstant(KeyUsageNumber usage, DerivedKeyType derivedKeyType)
        {
            // the "well-known constant" contains 5 bytes
            byte[] wellKnownConstant = new byte[5];

            // the first 4 bytes = usage number in big endian 
            byte[] usageBytes = BitConverter.GetBytes((int)usage);
        ...
        private static byte[] NFold(byte[] input, uint outputBits)
        {
            ...
            if (0 != outputBits % ConstValue.BYTE_SIZE)
            {
                throw new ArgumentException(
                    "The desired output length in bits should be a mutiply of 8 bits.");
            }
