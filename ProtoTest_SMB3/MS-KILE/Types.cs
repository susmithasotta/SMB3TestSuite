...
//
// Description: This file implements all PDU types. 
//              
//
//              
//------------------------------------------------------------------------------
...
namespace Microsoft.Protocols.TestTools.StackSdk.Security.Kile
{
    ...
    public class KileAsRequest : KilePdu
    {
        ...
        public override byte[] ToBytes()
        {
            Asn1BerEncodeBuffer asBerBuffer = new Asn1BerEncodeBuffer();
            asRequest.Encode(asBerBuffer, true);
            if (clientContext.TransportType == KileConnectionType.TCP)
            {
                return KileUtility.WrapLength(asBerBuffer.MsgCopy, true);
            }
            else
            {
                return asBerBuffer.MsgCopy;
            }
        }
        ...
        public override void FromBytes(byte[] buffer)
        {
            ...
            // Decode AS Request
            Asn1BerDecodeBuffer decodeBuffer = new Asn1BerDecodeBuffer(buffer);
            asRequest.Decode(decodeBuffer);
        }
    }
    ...
    public class KileAsResponse : KilePdu
    {
        ...
        public override byte[] ToBytes()
        {
            ...
            ticketEncPart.Encode(asBerBuffer, true);

            // Encrypt ticket enc_part to cipher data
            byte[] ticketCipherData = KileUtility.Encrypt(
                   (EncryptionType)serverContext.TicketEncryptKey.keytype.mValue,
                   serverContext.TicketEncryptKey.keyvalue.mValue,
                   asBerBuffer.MsgCopy,
                   (int)KeyUsageNumber.AS_REP_TicketAndTGS_REP_Ticket);
            asResponse.ticket.enc_part = new EncryptedData(serverContext.TicketEncryptKey.keytype.mValue,
                ticketCipherData);

            // Encode enc_part
            encPart.Encode(asBerBuffer, true);
            ...
            asResponse.enc_part = new EncryptedData((int)EncryptionType.RC4_HMAC, cipherData);

            // Encode AS Response
            asResponse.Encode(asBerBuffer, true);
        ...
        public override void FromBytes(byte[] buffer)
        {
            ...
            // Decode AS Response
            Asn1BerDecodeBuffer decodeBuffer = new Asn1BerDecodeBuffer(buffer);
            asResponse.Decode(decodeBuffer);

            // Get the current encryption type, cipher data, session key
            EncryptionType encryptType = (EncryptionType)asResponse.enc_part.etype.mValue;
            ...
            if (!(asResponse.padata != null && asResponse.padata.elements != null
                ...
                && (asResponse.padata.elements[0].padata_type.mValue == (long)PaDataType.PA_PK_AS_REP
                || asResponse.padata.elements[0].padata_type.mValue == (long)PaDataType.PA_PK_AS_REP_WINDOWS_OLD)))
            {
        ...
        public void Parse(byte[] asReplyKey)
        {
            ...
            byte[] cipherData = asResponse.enc_part.cipher.mValue;
            byte[] clearText = KileUtility.Decrypt(encryptType, asReplyKey, cipherData, keyUsageNumber);

            // Decode enc_part
            Asn1BerDecodeBuffer decodeBuffer = new Asn1BerDecodeBuffer(clearText);
            encPart = new EncASRepPart();
            encPart.Decode(decodeBuffer);
    ...
    public class KileTgsRequest : KilePdu
    {
        ...
        public override byte[] ToBytes()
        {
            Asn1BerEncodeBuffer tgsBerBuffer = new Asn1BerEncodeBuffer();
            tgsRequest.Encode(tgsBerBuffer, true);

            if (clientContext.TransportType == KileConnectionType.TCP)
            {
                return KileUtility.WrapLength(tgsBerBuffer.MsgCopy, true);
            }
            else
            {
                return tgsBerBuffer.MsgCopy;
            }
        }
        ...
        public override void FromBytes(byte[] buffer)
        {
            ...
            // Decode TGS Request
            tgsRequest.Decode(decodeBuffer);
            EncryptionType eType;
            byte[] cipherData;
            byte[] clearText;

            // Decode authentication header
            if (tgsRequest.padata != null
                && tgsRequest.padata.elements != null
                && tgsRequest.padata.elements.Length > 0)
            {
                AP_REQ authenticationHeader = new AP_REQ();
                decodeBuffer = new Asn1BerDecodeBuffer(tgsRequest.padata.elements[0].padata_value.mValue);
                authenticationHeader.Decode(decodeBuffer);

                // Decrypt Ticket from authentication header
                eType = (EncryptionType)authenticationHeader.ticket.enc_part.etype.mValue;
                cipherData = authenticationHeader.ticket.enc_part.cipher.mValue;
                byte[] sessionKey = serverContext.TicketEncryptKey.keyvalue.mValue;
                ...
                // Decode Ticket
                decodeBuffer = new Asn1BerDecodeBuffer(clearText);
                tgtTicket = new EncTicketPart();
                tgtTicket.Decode(decodeBuffer);

                // Decrypt Authenticator from authentication header
                eType = (EncryptionType)authenticationHeader.authenticator.etype.mValue;
                cipherData = authenticationHeader.authenticator.cipher.mValue;
                clearText = KileUtility.Decrypt(
                    eType,
                    serverContext.TgsSessionKey.keyvalue.mValue,
                    cipherData,
                ...
                authenticator.Decode(decodeBuffer);
            }
            ...
            if (tgsRequest.req_body.enc_authorization_data != null
                ...
                && tgsRequest.req_body.enc_authorization_data.cipher.mValue != null)
            {
                eType = (EncryptionType)tgsRequest.req_body.enc_authorization_data.etype.mValue;
                cipherData = tgsRequest.req_body.enc_authorization_data.cipher.mValue;

                if (authenticator != null && authenticator.subkey != null)
                {
                    clearText = KileUtility.Decrypt(
                        eType,
                        authenticator.subkey.keyvalue.mValue,
                        cipherData,
                ...
                encAuthorizationData.Decode(decodeBuffer);
            }
        }
    }
    ...
    public class KileTgsResponse : KilePdu
    {
        ...
        public override byte[] ToBytes()
        {
            ...
            // Encode ticket enc_part
            ticketEncPart.Encode(tgsBerBuffer, true);

            // Encrypt ticket enc_part to cipher data
            byte[] ticketCipherData = KileUtility.Encrypt(
                   (EncryptionType)serverContext.TicketEncryptKey.keytype.mValue,
                   serverContext.TicketEncryptKey.keyvalue.mValue,
                   tgsBerBuffer.MsgCopy,
                   (int)KeyUsageNumber.AS_REP_TicketAndTGS_REP_Ticket);
            tgsResponse.ticket.enc_part = new EncryptedData(serverContext.TicketEncryptKey.keytype.mValue,
                ticketCipherData);

            // Encode enc_part
            encPart.Encode(tgsBerBuffer, true);
            ...
            if (serverContext.TgsSubSessionKey != null)
            {
                ...
                cipherData = KileUtility.Encrypt((EncryptionType)eKey.keytype.mValue,
                    eKey.keyvalue.mValue,
                    tgsBerBuffer.MsgCopy,
            ...
            tgsResponse.enc_part = new EncryptedData(eKey.keytype.mValue, cipherData);

            // Encode TGS Response
            tgsResponse.Encode(tgsBerBuffer, true);
        ...
        public override void FromBytes(byte[] buffer)
        {
            ...
            // Decode TGS Response
            Asn1BerDecodeBuffer decodeBuffer = new Asn1BerDecodeBuffer(buffer);
            tgsResponse.Decode(decodeBuffer);

            // Get the current encryption type, cipher data
            EncryptionType encryptType = (EncryptionType)tgsResponse.enc_part.etype.mValue;
            byte[] cipherData = tgsResponse.enc_part.cipher.mValue;

            // Decrypt enc_part to clear text
            byte[] clearText = KileUtility.Decrypt(
                encryptType,
                clientContext.TgsSessionKey.keyvalue.mValue,
                cipherData,
                (int)KeyUsageNumber.TGS_REP_encrypted_part);
    ...
    public class KileApRequest : KilePdu
    {
        ...
        public override byte[] ToBytes()
        {
            ...
            {
                Asn1BerEncodeBuffer asnBuffPlainAuthenticator = new Asn1BerEncodeBuffer();
                authenticator.Encode(asnBuffPlainAuthenticator, true);

                apRequest.authenticator = new EncryptedData();
                apRequest.authenticator.etype = new Int32(0);
                byte[] encAsnEncodedAuth = asnBuffPlainAuthenticator.MsgCopy;
                if (kileContext.ApSessionKey != null && kileContext.ApSessionKey.keytype != null
                    && kileContext.ApSessionKey.keyvalue != null
                    && kileContext.ApSessionKey.keyvalue.mValue != null)
                {
                    encAsnEncodedAuth = KileUtility.Encrypt((EncryptionType)kileContext.ApSessionKey.keytype.mValue,
                                                            kileContext.ApSessionKey.keyvalue.mValue,
                                                            asnBuffPlainAuthenticator.MsgCopy,
                                                            (int)KeyUsageNumber.AP_REQ_Authenticator);
                    apRequest.authenticator.etype = new Int32(kileContext.ApSessionKey.keytype.mValue);
                }

                apRequest.authenticator.cipher = new Asn1OctetString(encAsnEncodedAuth);
            }

            Asn1BerEncodeBuffer apBerBuffer = new Asn1BerEncodeBuffer();
            apRequest.Encode(apBerBuffer, true);
        ...
        public void FromBytes(byte[] buffer, EncryptionKey ticketEncryptKey)
        {
            ...
            // Decode AP Request
            Asn1BerDecodeBuffer decodeBuffer = new Asn1BerDecodeBuffer(apBody);
            apRequest.Decode(decodeBuffer);

            if (apRequest.ticket != null)
            {
                // Decrypt Ticket
                EncryptionType eType = (EncryptionType)apRequest.ticket.enc_part.etype.mValue;
                byte[] cipherData = apRequest.ticket.enc_part.cipher.mValue;
                byte[] clearText = KileUtility.Decrypt(eType, ticketEncryptKey.keyvalue.mValue, cipherData,
                        (int)KeyUsageNumber.AS_REP_TicketAndTGS_REP_Ticket);

                // Decode Ticket
                decodeBuffer = new Asn1BerDecodeBuffer(clearText);
                ticket = new EncTicketPart();
                ticket.Decode(decodeBuffer);

                if (apRequest.authenticator != null)
                {
                    // Decrypt Authenticator
                    eType = (EncryptionType)apRequest.authenticator.etype.mValue;
                    cipherData = apRequest.authenticator.cipher.mValue;
                    clearText = KileUtility.Decrypt(eType, ticket.key.keyvalue.mValue, cipherData,
                        (int)KeyUsageNumber.AP_REQ_Authenticator);
    ...
    public class KileApResponse : KilePdu
    {
        ...
        public override void FromBytes(byte[] buffer)
        {
            ...
            // Decode AP response
            Asn1BerDecodeBuffer decodeBuffer = new Asn1BerDecodeBuffer(apBody);
            apResponse.Decode(decodeBuffer);

            // Get the current encryption type, cipher data
            EncryptionType encryptType = (EncryptionType)apResponse.enc_part.etype.mValue;
            byte[] cipherData = apResponse.enc_part.cipher.mValue;
            byte[] sessionKey = kileContext.ApSessionKey.keyvalue.mValue;
        ...
        public override byte[] ToBytes()
        {
            ...
            {
                // Encode enc_part
                apEncPart.Encode(apBerBuffer, true);
                byte[] encAsnEncoded = apBerBuffer.MsgCopy;
                ...
                // Encrypt enc_part
                EncryptionType eType = (EncryptionType)key.keytype.mValue;
                byte[] cipherData = KileUtility.Encrypt(
                    eType,
                    key.keyvalue.mValue,
                    apBerBuffer.MsgCopy,
                    (int)KeyUsageNumber.AP_REP_EncAPRepPart);
                apResponse.enc_part = new EncryptedData((int)eType, cipherData);
            }

            // Encode AP Response
            apResponse.Encode(apBerBuffer, true);
    ...
    public class KrbZero : KilePdu
    {
        ...
        public KrbZero(KileClientContext clientContext) : base(clientContext)
        {
        }
    }
    ...
    public class KrbCred : KilePdu
    {
        ...
        public override byte[] ToBytes()
        {
            if (credEncPart != null)
            {
                Asn1BerEncodeBuffer encKrbCredBuf = new Asn1BerEncodeBuffer();
                credEncPart.Encode(encKrbCredBuf);

                byte[] encryptedData = encKrbCredBuf.MsgCopy;
                if (clientContext.ContextKey != null && clientContext.ContextKey.keytype != null
                    && clientContext.ContextKey.keyvalue != null && clientContext.ContextKey.keyvalue.mValue != null)
                {
                    encryptedData = KileUtility.Encrypt((EncryptionType)clientContext.ContextKey.keytype.mValue,
                                                        clientContext.ContextKey.keyvalue.mValue,
                                                        encKrbCredBuf.MsgCopy,
                                                        (int)KeyUsageNumber.KRB_CRED_EncPart);
                    krbCred.enc_part = new EncryptedData(clientContext.ContextKey.keytype.mValue, encryptedData);
                }
                else
                {
                    krbCred.enc_part = new EncryptedData(0, encryptedData);
                }
            }

            Asn1BerEncodeBuffer krbCredBuf = new Asn1BerEncodeBuffer();
            krbCred.Encode(krbCredBuf);
            return KileUtility.WrapLength(krbCredBuf.MsgCopy, true);
        }
    }
    ...
    public class KrbPriv : KilePdu
    {
        ...
        public override byte[] ToBytes()
        {
            ...
            if (privEncPart != null)
            {
                Asn1BerEncodeBuffer encKrbPrivBuf = new Asn1BerEncodeBuffer();
                privEncPart.Encode(encKrbPrivBuf);

                byte[] encryptedData = encKrbPrivBuf.MsgCopy;
                if (clientContext.ContextKey != null && clientContext.ContextKey.keytype != null
                    && clientContext.ContextKey.keyvalue != null && clientContext.ContextKey.keyvalue.mValue != null)
                {
                    encryptedData = KileUtility.Encrypt((EncryptionType)clientContext.ContextKey.keytype.mValue,
                                                        clientContext.ContextKey.keyvalue.mValue,
                                                        encKrbPrivBuf.MsgCopy,
                                                        (int)KeyUsageNumber.KRB_PRIV_EncPart);
                    krbPriv.enc_part = new EncryptedData(clientContext.ContextKey.keytype.mValue, encryptedData);
                }
                else
                {
                    krbPriv.enc_part = new EncryptedData(0, encryptedData);
                }
            }

            Asn1BerEncodeBuffer krbPrivBuf = new Asn1BerEncodeBuffer();
            krbPriv.Encode(krbPrivBuf, true);
            return KileUtility.WrapLength(krbPrivBuf.MsgCopy, true);
        }
    }
    ...
    public class KileKrbError : KilePdu
    {
        ...
        public override void FromBytes(byte[] buffer)
        {
            ...
            // Decode Krb Error
            Asn1BerDecodeBuffer decodeBuffer = new Asn1BerDecodeBuffer(buffer);
            krbError.Decode(decodeBuffer);

            // Get its error code
            krbErrorCode = (KRB_ERROR_CODE)krbError.error_code.mValue;
        }
        ...
        public override byte[] ToBytes()
        {
            Asn1BerEncodeBuffer errorBerBuffer = new Asn1BerEncodeBuffer();
            krbError.Encode(errorBerBuffer, true);

            if (kileContext.TransportType == KileConnectionType.TCP)
            {
                return KileUtility.WrapLength(errorBerBuffer.MsgCopy, true);
            }
            else
            {
                return errorBerBuffer.MsgCopy;
            }
        }
    }
    ...
    public enum KRB_ERROR_CODE : int
    {
        ...
        /// <summary>
        ///  Requested starttime islater than end time
        /// </summary>
        ...
        /// <summary>
        /// KDC has no support fo r  checksum type
        /// </summary>
        ...
        /// <summary>
        /// Requested server and ticketdon't match
        /// </summary>
    ...
    public class Token4121 : KilePdu
    {
        ...
        public override void FromBytes(byte[] buffer)
        {
            ...
            byte[] cipher = ArrayUtility.SubArray(tokenBody, Marshal.SizeOf(typeof(TokenHeader4121)));
            ...
            {
                // TODO: maybe a windows bug, the rrc actually is 44.
                if ((kileContext.ChecksumFlag & ChecksumFlags.GSS_C_DCE_STYLE) == ChecksumFlags.GSS_C_DCE_STYLE)
                {
                    rrc = 44;
                }
            ...
            if (tokId == TOK_ID.Wrap4121 && (tokenHeader.flags & WrapFlag.Sealed) == WrapFlag.Sealed)
            {
                ...
                byte[] plainBuffer = KileUtility.Decrypt((EncryptionType)key.keytype.mValue,
                                                       key.keyvalue.mValue,
                                                       cipher,
                                                       (int)keyUsage);
            ...
            else   // no confidentiality is provided
            {
                ...
                if ((EncryptionType)key.keytype.mValue == EncryptionType.AES128_CTS_HMAC_SHA1_96)
                {
        ...
        public override byte[] ToBytes()
        {
            ...
            if (tokenHeader.tok_id == TOK_ID.Wrap4121 && (tokenHeader.flags & WrapFlag.Sealed) == WrapFlag.Sealed)
            {
                ...
                cipher = KileUtility.Encrypt((EncryptionType)key.keytype.mValue,
                                             key.keyvalue.mValue,
                                             plainBuf,
                                             (int)keyUsage);
            }
            else   // no confidentiality is provided or mic token
            {
                ...
                cipher = KileUtility.GetChecksum(key.keyvalue.mValue, plainBuf, (int)keyUsage, checksumType);
    ...
    public class Token1964_4757 : KilePdu
    {
        ...
        public override void FromBytes(byte[] buffer)
        {
            ...
            {
                // the tokenBody must > header + sequnce number + checksum + confounder + at least one pad
                if (tokenBody.Length <
                (ConstValue.HEADER_FIRST_8_BYTE_SIZE + ConstValue.CHECKSUM_SIZE_RFC1964 + ConstValue.SEQUENCE_NUMBER_SIZE
                 + ConstValue.CONFOUNDER_SIZE + 1))
                {
                    throw new FormatException("The token body is incomplete!");
                }
                cipher = ArrayUtility.SubArray(tokenBody,
                                               ConstValue.HEADER_FIRST_8_BYTE_SIZE
                                               + ConstValue.CHECKSUM_SIZE_RFC1964
                                               + ConstValue.SEQUENCE_NUMBER_SIZE);
            }
            ...
            bool isExport = (EncryptionType)key.keytype.mValue == EncryptionType.RC4_HMAC_EXP;
            byte[] plainText = data;
            if (tokenHeader.seal_alg == SEAL_ALG.DES)
            {
                ...
                for (int i = 0; i < 8; i++)
                {
                    Klocal[i] = (byte)(key.keyvalue.mValue[i] ^ 0xF0);
                }
            ...
            if (tokenHeader.tok_id == TOK_ID.Wrap1964_4757)
            {
                ...
                if (pad > 8 || pad < 1)
                {
                    throw new FormatException("The token padding is incorrect!");
                }
                for (int i = 1; i < pad; ++i)
                {
                ...
                data = ArrayUtility.SubArray(plainText,
                                             ConstValue.CONFOUNDER_SIZE,
                                             plainText.Length - ConstValue.CONFOUNDER_SIZE - pad);
            }
            #endregion decrypt data

            #region verify checksum
            byte[] check = null;
            byte[] toChecksumText = ArrayUtility.ConcatenateArrays(header,
                                                                  plainText);
            if ((EncryptionType)key.keytype.mValue == EncryptionType.RC4_HMAC
                || (EncryptionType)key.keytype.mValue == EncryptionType.RC4_HMAC_EXP)
            {
            ...
            switch (tokenHeader.sng_alg)
            {
                case SGN_ALG.DES_MAC_MD5:
                    check = KileUtility.DesCbcMac(key.keyvalue.mValue, new byte[ConstValue.DES_BLOCK_SIZE], md5);
                    break;

                case SGN_ALG.MD2_5:
                    check = KileUtility.MD2_5(key.keyvalue.mValue, toChecksumText);
                    break;
                ...
                case SGN_ALG.HMAC:
                    byte[] keySign = KileUtility.HMAC(key.keyvalue.mValue, ConstValue.SIGNATURE_KEY);
            ...
            switch ((EncryptionType)key.keytype.mValue)
            {
                ...
                case EncryptionType.DES_CBC_MD5:
                    ...
                    tokenHeader.snd_seq = KileUtility.DesCbcDecrypt(key.keyvalue.mValue,
                ...
                case EncryptionType.RC4_HMAC_EXP:
                    ...
                    tokenHeader.snd_seq = KileUtility.RC4HMAC(key.keyvalue.mValue,
                                                              tokenHeader.sng_cksum,
                                                              sequence,
                                                              isExport);
                    break;
        ...
        public override byte[] ToBytes()
        {
            ...
            switch ((EncryptionType)key.keytype.mValue)
            {
                ...
                case EncryptionType.DES_CBC_MD5:
                    ...
                    seqBuf = SetSequenceBuffer(seqNumber, kileContext.isInitiator);
                    tokenHeader.snd_seq = KileUtility.DesCbcEncrypt(key.keyvalue.mValue,
                                                                    tokenHeader.sng_cksum,
                                                                    seqBuf);
                    break;
    ...
    /// </summary>
    public enum DerivedKeyType : byte
    {
        /// <summary>
    ...
    public enum PrincipalType : int
    {
        ...
        NT_SMTP_NAME = 7,
        /// <summary>
        /// Enterprise name - may be mapped to principalname
        /// </summary>
        NT_ENTERPRISE = 10
    }
