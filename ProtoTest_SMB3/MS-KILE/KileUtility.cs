...
//
// Description: This file implements a static class that provides some helper  
//              functions.
//
//              
//------------------------------------------------------------------------------
...
namespace Microsoft.Protocols.TestTools.StackSdk.Security.Kile
{
    ...
    public static class KileUtility
    {
        ...
        /// from response message. The usage indicated in secion 7 of RFC4210 
        ...
        internal static byte[] Encrypt(EncryptionType type, byte[] sessionKey, byte[] plainData, int usage)
        {
            switch (type)
            {
                case EncryptionType.AES128_CTS_HMAC_SHA1_96:
                    return AesCtsHmacSha1Crypto.Encrypt(sessionKey, plainData,
                        (KeyUsageNumber)usage, AesKeyType.Aes128BitsKey);

                case EncryptionType.AES256_CTS_HMAC_SHA1_96:
                    return AesCtsHmacSha1Crypto.Encrypt(sessionKey, plainData,
                        (KeyUsageNumber)usage, AesKeyType.Aes256BitsKey);

                case EncryptionType.DES_CBC_CRC:
                    return DesCbcCrypto.Encrypt(sessionKey, plainData,
                        (KeyUsageNumber)usage, EncryptionType.DES_CBC_CRC);

                case EncryptionType.DES_CBC_MD5:
                    return DesCbcCrypto.Encrypt(sessionKey, plainData,
                        (KeyUsageNumber)usage, EncryptionType.DES_CBC_MD5);

                case EncryptionType.RC4_HMAC:
                    return Rc4HmacCrypto.Encrypt(sessionKey, plainData,
                        (KeyUsageNumber)usage, EncryptionType.RC4_HMAC);

                case EncryptionType.RC4_HMAC_EXP:
                    return Rc4HmacCrypto.Encrypt(sessionKey, plainData,
                        (KeyUsageNumber)usage, EncryptionType.RC4_HMAC_EXP);
        ...
        /// in secion 7 of RFC4210 and section 3 of RFC4757 is used to derive key 
        ...
        internal static byte[] Decrypt(EncryptionType type, byte[] sessionKey, byte[] cipherData, int usage)
        {
            ...
            {
                case EncryptionType.AES128_CTS_HMAC_SHA1_96:
                    return AesCtsHmacSha1Crypto.Decrypt(sessionKey, cipherData,
                        (KeyUsageNumber)usage, AesKeyType.Aes128BitsKey);
                ...
                case EncryptionType.DES_CBC_CRC:
                    return DesCbcCrypto.Decrypt(sessionKey, cipherData,
                        (KeyUsageNumber)usage, EncryptionType.DES_CBC_CRC);
                ...
                case EncryptionType.RC4_HMAC:
                    return Rc4HmacCrypto.Decrypt(sessionKey, cipherData,
                        (KeyUsageNumber)usage, EncryptionType.RC4_HMAC);
        ...
        internal static _SeqOfKerberosString String2SeqKerbString(params string[] sourceString)
        {
            ...
            _SeqOfKerberosString seqString = new _SeqOfKerberosString(kerberosStringList.ToArray());
            return seqString;
        }
        ...
        {
            EncryptionType encryptType = (EncryptionType)ticket.enc_part.etype.mValue;
            byte[] clearText = KileUtility.Decrypt(
                      encryptType,
                      key,
                      ticket.enc_part.cipher.mValue,
            ...
            encTicketPart.Decode(decodeBuffer);
        ...
        public static void UpdateAuthDataInTicket(Ticket ticket, byte[] key, AuthorizationData authorizationData)
        {
            ...
            Asn1BerEncodeBuffer ticketBerBuffer = new Asn1BerEncodeBuffer();
            encTicketPart.Encode(ticketBerBuffer, true);

            byte[] cipherData = KileUtility.Encrypt(
                encryptType,
                key,
                ticketBerBuffer.MsgCopy,
                (int)KeyUsageNumber.AS_REP_TicketAndTGS_REP_Ticket);
            ticket.enc_part = new EncryptedData((int)encryptType, cipherData);
        }
        ...
        internal static void Sign(KileRole kileRole, params SecurityBuffer[] securityBuffers)
        {
            ...
            byte[] padding = SspiUtility.ConcatenateSecurityBuffers(securityBuffers, SecurityBufferType.Padding);
            message = ArrayUtility.ConcatenateArrays(message, padding);
        ...
        internal static bool Verify(KileRole kileRole, params SecurityBuffer[] securityBuffers)
        {
            ...
            if (token.Length == 0)
            {
                throw new ArgumentException("No token can be verified in SecurityBuffer array.");
            }

            KilePdu pdu;
            return kileRole.GssVerifyMic(token, message, out pdu);
        }
        ...
        {
            byte[] cipherData = SspiUtility.ConcatenateReadWriteSecurityBuffers(securityBuffers, SecurityBufferType.Data);
            byte[] token = SspiUtility.ConcatenateSecurityBuffers(securityBuffers, SecurityBufferType.Token);
            ...
            KilePdu pdu = kileRole.GssUnWrap(ArrayUtility.ConcatenateArrays(token, cipherData, padding));
        ...
        private static SGN_ALG GetSgnAlg(KileRole kileRole)
        {
            ...
            EncryptionType type = (EncryptionType)key.keytype.mValue;
