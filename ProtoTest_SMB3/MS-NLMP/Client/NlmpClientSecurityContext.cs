...
namespace Microsoft.Protocols.TestTools.StackSdk.Security.Nlmp
{
    ...
    public class NlmpClientSecurityContext : ClientSecurityContext
    {
        ...
        /// in connection-oriented mode this packet is the first packet that sended by client.
        /// </summary>
        ...
        /// <param name="isUnicode">whether use unicode encoding</param>
        ...
        /// </param>
        /// <param name="implicitNtlmSessionKey">output the session key for implicit ntlm</param>
        ...
        public void GetSecurityToken(
            ...
            out byte[] implicitNtlmSessionKey,
            ...
            out byte[] caseSensitivePassword)
        {
            ...
            if (NlmpUtility.IsNtlmV2(ntlmVersion))
            {
                List<AV_PAIR> pairs = new List<AV_PAIR>();

                byte[] serverName = NlmpUtility.StringGetBytes(Environment.MachineName, isUnicode);

                NlmpUtility.AddAVPair(pairs, AV_PAIR_IDs.MsvAvNbComputerName, (ushort)serverName.Length, serverName);
            ...
            NegotiateTypes nlmpFlags = new NegotiateTypes();

            // primary flags
            nlmpFlags |= NegotiateTypes.NTLMSSP_REQUEST_TARGET;
            nlmpFlags |= NegotiateTypes.NTLMSSP_NEGOTIATE_NTLM;
            nlmpFlags |= NegotiateTypes.NTLMSSP_NEGOTIATE_UNICODE;

            // second nlmpFlags
            nlmpFlags |= NegotiateTypes.NTLMSSP_NEGOTIATE_VERSION;

            // the following is optional
            nlmpFlags |= NegotiateTypes.NTLMSSP_NEGOTIATE_128;
            nlmpFlags |= NegotiateTypes.NTLMSSP_NEGOTIATE_56;
            ...
            implicitNtlmSessionKey = new byte[sessionBaseKey.Length + caseSensitivePassword.Length];

            Array.Copy(sessionBaseKey, implicitNtlmSessionKey, sessionBaseKey.Length);

            Array.Copy(
                caseSensitivePassword, 0, implicitNtlmSessionKey, sessionBaseKey.Length, caseSensitivePassword.Length);
        }
        ...
        {
            InitializeEncryptedRandomKey(
                flags, challenge, lmChallengeResponse, responseKeyLM, out exportedSessionKey,
                out encryptedRandomSessionKey);

            InitializeKeys(flags, exportedSessionKey);
            ...
            )
        {
            ...
            byte[] responseKeyNT = NlmpUtility.GetResponseKeyNt(
                this.client.Config.Version, this.currentActiveCredential.DomainName, this.currentActiveCredential.AccountName,
                this.currentActiveCredential.Password);
        ...
        /// <param name="responseKeyNT">the resonse nt key</param>
        ...
        /// <summary>
        /// update the lm challenge reponse
        /// </summary>
        /// <param name="targetInfo">the challenge packet</param>
        /// <param name="lmChallengeResponse">the lm challenge resonse</param>
        ...
        /// <summary>
        /// initialize the encrypted randomkey
        /// </summary>
        /// <param name="flags">the flag for challenge</param>
        /// <param name="challenge">the challenge packet</param>
        /// <param name="lmChallengeResponse">the lm challenge reponse bytes</param>
        /// <param name="responseKeyLM">the response key lm bytes</param>
        /// <param name="exportedSessionKey">the exported session key</param>
        /// <param name="encryptedRandomSessionKey">the encrypted random session key</param>
        private void InitializeEncryptedRandomKey(
            NegotiateTypes flags,
            NlmpChallengePacket challenge,
            byte[] lmChallengeResponse,
            byte[] responseKeyLM,
        ...
        /// <summary>
        /// to initialize the RC4(H, K).
        /// </summary>
        private void InitializeRC4()
        {
        ...
        /// <summary>
        /// initialize the keys in context
        /// </summary>
        /// <param name="flags">the flag for challenge</param>
        /// <param name="exportedSessionKey">the exported session key to generated keys in context</param>
        private void InitializeKeys(
            NegotiateTypes flags,
