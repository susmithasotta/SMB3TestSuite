    ...
    public static class SspiUtility
    {
            ...
            bool bothReadOnlyAndReadWrite)
        {
            ...
            for (int i = 0; i < securityBuffers.Length; i++)
            {
                ...
                if (securityBufferType == targetType)
                {
        ...
        /// </exception>
        public static byte[] ConcatenateSecurityBuffers(SecurityBuffer[] securityBuffers, SecurityBufferType targetType)
        {
            ...
            return ConcatenateSecurityBuffers(securityBuffers, targetType, true);
        }
            ...
            SecurityBufferType targetType)
        {
            ...
            return ConcatenateSecurityBuffers(securityBuffers, targetType, false);
        }


        /// <summary>
        /// Update buffers of a specified type in the list. 
        /// Buffer will be seperated automatically to fit the original length of a security buffer. 
        ...
        public static void UpdateSecurityBuffers(SecurityBuffer[] securityBuffers, SecurityBufferType targetType, byte[] buffer)
        {
            ...
            for (int i = 0; i < securityBuffers.Length; i++)
            {
                ...
                bool isReadOnly = ((securityBuffers[i].BufferType & SecurityBufferType.ReadOnly) != 0)
                               || ((securityBuffers[i].BufferType & SecurityBufferType.ReadOnlyWithChecksum) != 0);
                if (securityBufferType == targetType && !isReadOnly)
                {
        ...
        internal static object QueryContextAttributes(ref SecurityHandle contextHandle, string contextAttribute)
        {
            ...
            string stringAttribute = contextAttribute.ToUpper();
            ...
            if (hResult == NativeMethods.SEC_E_OK)
            {
                ...
                switch (stringAttribute)
                {
                    ...
                    case SECPKG_ATTR_SESSION_KEY:
                        ...
                        break;
                    case SECPKG_ATTR_PACKAGE_INFO:
                        SspiSecurityPackageInfomation packageInfo;

                        packageInfo = (SspiSecurityPackageInfomation)Marshal.PtrToStructure(
                            Marshal.ReadIntPtr(pBuffer),
                            typeof(SspiSecurityPackageInfomation));
                        attributeValue = new SecurityPackageInfomation(packageInfo);
                        break;
                    case SECPKG_ATTR_NEGOTIATION_INFO:
