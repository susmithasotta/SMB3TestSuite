    ...
    public class MessageUtils : IDisposable
    {
        ...
        public void Initialize<T>(ref T value)
        {
            lock (messageUtilsLock)
            {
                                ...
                                t.GetFields(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance))
                {
                    ...
                    else if (ft.IsArray || IsSequence(ft))
                    {
                        int size, length;
            ...
            foreach (FieldInfo field in fields)
            {
                ...
                if (ft.IsArray || IsSequence(ft))
                {
                    ...
                    if (fv != null)
                    {
                        ...
                        if (actualSize > size)
                            host.Assert(false,
                                "validating value of type '{0}': advocated size of field '{1} is smaller than actual size (advocated == {2}, actual == {3}",
        ...
        public static bool CheckValueByRange(object inValue, PossibleValueRangeAttribute possibleValueAttribute)
        {
            ...
            switch (typeName)
            {
                ...
                case "Decimal":
                    ...
                    break;
                default:
                    throw new InvalidOperationException(String.Format("{0} data type is not supported", typeName));
            }
                           ...
                           where T : IComparable<T>
        {
            try
            {
                ...
                {
                    throw new InvalidOperationException(String.Format("max value: {0} could not be less than min value: {1}", maxValue.ToString(), minValue.ToString()));
                }
            ...
            catch (FormatException ex)
            {
               ...
               throw new InvalidOperationException(String.Format("Format exception happens when convert {0} or {1} to {2}, information: {3}",
                            possibleValue.MinValue, possibleValue.MaxValue,
                            inValue.GetType().Name, ex.ToString()));
            }
            catch (OverflowException ex)
            {     
                throw new InvalidOperationException(String.Format("{0} or {1} overflowed as {2}, information: {3}", possibleValue.MinValue,
                        possibleValue.MaxValue, inValue.GetType().Name, ex.ToString()));
            }
            catch (Exception ex)
            {
                throw new InvalidOperationException(String.Format("Error happens when check value range, information: {0}", ex.ToString()));
            }
        ...
        static object GetValueFromString(Type dataType, string inputValue)
        {
            ...
            {
                return (object)Double.Parse(inputValue);
            }

            if (dataType == typeof(Single))
            {
                return (object)Single.Parse(inputValue);
            }

            if (dataType == typeof(Decimal))
            {
                return (object)Decimal.Parse(inputValue);
            }
        ...
        public string ToString(object value)
        {
            ...
            else if (t.IsEnum)
            {
                ...
                try
                {
                    object uvalue = Convert.ChangeType(value, Enum.GetUnderlyingType(t));
        ...
        {
            ulong targetVal = (ulong)Convert.ChangeType(target, TypeCode.UInt64);
            ulong enumVal = (ulong)Convert.ChangeType(enumValue, TypeCode.UInt64);
