...
namespace Microsoft.Protocols.TestTools.StackSdk.Messages.Runtime.Marshaling
{
    ...
    public class Marshaler : IDisposable, IEvaluationContext
    {
        ...
        {
            string failMessage = parameters.Length > 0 ? String.Format(message, parameters) : message;
            if (desc.Type != null)
            {
                failMessage = String.Format("while processing '{0}': {1}", desc, failMessage);
            }
            return failMessage;
        }
        ...
        public void TestAssertFail(string message, params object[] parameters)
        {
            if (host != null)
            {
                host.Assert(false, GetMessage(new MarshalingDescriptor(), message, parameters));
            }
            else
            {
                Console.WriteLine("FAIL: " + message);
        ...
        public void TestAssumeFail(MarshalingDescriptor desc, string message, params object[] parameters)
        {
            if (host != null)
            {
                host.Assume(false, GetMessage(desc, message, parameters));
            }
            else
            {
                Console.WriteLine("FAIL: " + message);
        ...
        public void TestAssumeFail(string message, params object[] parameters)
        {
            if (host != null)
            {
                host.Assume(false, GetMessage(new MarshalingDescriptor(), message, parameters));
            }
            else
            {
                Console.WriteLine("FAIL: " + message);
        ...
        public void DebugFail(string message, params object[] parameters)
        {
            if (host != null)
            {
                host.Debug(false, GetMessage(new MarshalingDescriptor(), message, parameters));
            }
            else
            {
                Console.WriteLine("FAIL: " + message);
        ...
        public void Trace(string message, params object[] parameters)
        {
            if (tracing)
            {
                ...
                {
                    Console.WriteLine("trace: " + String.Format(message, parameters));
                }
            }
        }
        ...
        public bool TryResolveCustomType(string typeName, out string size)
        {
            ...
            if (customTypes.TryGetValue(typeName, out type))
            {
                ...
                size = msr.GetSize(this, descriptor, null).ToString();
                return true;
            }
            else
            {
                ...
                if (pointerSymbolStore.TryGetValue(variable, out valueObj))
                {
                    try
                    {
                        ...
                        else
                        {
                            ...
                            if (vt.IsArray)
                            {
                                value = Convert.ToInt32(((Array)valueObj).GetValue(0));
                                pointerValue = 1;
                            }
                            else
                            {
