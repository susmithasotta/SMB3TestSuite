                ...
                if (forwardMap.TryGetValue(id, out oldTarget))
                {
                    if (!Object.Equals(oldTarget, target))
                    {
                        if (host != null)
                        {
                            ...
                            Console.WriteLine(String.Format("failed binding identifier '{0}' to '{1}': identifier already bound to '{2}'",
                                    id, target, oldTarget));
                    }
                }
                else
                ...
                if (backwardMap.TryGetValue(target, out oldId))
                {
                    if (id != oldId)
                    {
                        if (host != null)
                        {
                            ...
                            Console.WriteLine(String.Format("failed binding identifier '{0}' to '{1}': target already uses identifier '{2}'",
                                        id, target, oldId));
                    }
                }
                else
            ...
            lock (identifierBindingLock)
            {
                ...
                if (!backwardMap.TryGetValue(target, out id))
                {
                    if (host != null)
                    {
                        ...
                        Console.WriteLine(String.Format("failed resolving target '{0}': not bound to identifier", target));
                }
                return id;
            }
        }
            ...
            lock (identifierBindingLock)
            {
                ...
                if (!forwardMap.TryGetValue(id, out t))
                {
                    if (host != null)
                    {
                        ...
                        Console.WriteLine(String.Format("failed resolving identifier '{0}': not bound to target", id));
                }
                return t;
            }
        }
